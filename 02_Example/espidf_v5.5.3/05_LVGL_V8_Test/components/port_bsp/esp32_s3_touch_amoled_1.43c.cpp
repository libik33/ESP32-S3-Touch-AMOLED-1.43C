...
void my_lvgl_indev_cb(lv_indev_drv_t *drv, lv_indev_data_t *indevData) {
    uint16_t tp_x = 0x00;
    uint16_t tp_y = 0x00;
    if(GetCoords(&tp_x,&tp_y)) {
        indevData->point.x = LCD_WIDTH - tp_x;
...
bsp_lvgl_t bsp_broolesia_display_init(void) {
    ...
    disp_cfg.profile.buffer_height = 100; // 设置缓冲区高度，单位为像素行，根据实际情况调整
    ...
    indev.disp    = disp;
    indev.read_cb = my_lvgl_indev_cb;
